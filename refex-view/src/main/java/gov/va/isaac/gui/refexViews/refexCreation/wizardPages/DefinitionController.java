...
public class DefinitionController implements PanelControllersI {
	...
	public void initialize() {
		...
		parentConcept.set(new SimpleDisplayConcept(RefexDynamic.DYNAMIC_SEMEME_IDENTITY.getUuids()[0].toString()));
	...
	public boolean checkParentHierarchy() {
		try 
		{
			if (!parentConcept.getConcept().isKindOf(OTFUtility.getConceptVersion(RefexDynamic.DYNAMIC_SEMEME_IDENTITY.getNid()))) 
			{
