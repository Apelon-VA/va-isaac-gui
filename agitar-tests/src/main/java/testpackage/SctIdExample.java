...
/**
 * {@link SctIdExample} contains validation utilities for SNOMED ID (SCT ID). A unique long
 ...
 * 
 */
//TODO integrate this class down lower in the ISAAC hierarchy, when necessary
public class SctIdExample
{
	...
	private static int[] InverseD5_ = {0, 4, 3, 2, 1, 5, 6, 7, 8, 9 };
...
//	}
	...
	public static boolean isValidSctId(String idAsString)
	{
		...
		for (int i = idAsString.length() - 1; i >= 0; i--)
		{
			check = Dihedral_[check][FnF_[(idAsString.length() - i - 1) % 8][new Integer(new String(new char[] { idAsString.charAt(i) }))]];
		}
		if (check != 0)
		{
			return false;
